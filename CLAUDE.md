...
`docs/API-DOC-auto-app-push.md` is the contract for `ecs-api` — the same payload shape and envelope also serve `express` (`be-push-notification-tool`), which reimplements it at a different path; that repo's own docs are the contract source for its divergences (see the 404 bullet below). `execute()` in `PushConsole.tsx` builds the payload with `buildPayload` and posts it through `submitAutoAppPush` (`src/lib/api.ts`, now taking `server` as a third argument), which picks the target by server:

- **`ecs-api`** — through our own route handler at `src/app/api/push/auto-app-push/route.ts`, which forwards to `POST {ECS_API_URL}/api/test_notification/auto_app_pushes`. When the route handler itself has a problem (env not set, no token on the request, body not JSON, host unreachable) it answers with the same `{ error: { code, title, message, errors[] } }` object as the API's failures (the house shape from the EMO API Specification Summary sheet, repeated in `docs/API-DOC-auto-app-push.md`) — `title` is the headline, `message` the sentence under it, `errors[]` raw detail — so `src/lib/api.ts` reads both the same way.
- **`express`** — straight from the browser to `POST {NEXT_PUBLIC_EXPRESS_API_URL}/api/notifications/auto-app-pushes`, no proxy hop. This is a deliberate asymmetry, not an oversight: see the CORS paragraph below.
...
- Servers: `ecs-api` (the old Rails batch path, proxied through our route handler) and `express` (`be-push-notification-tool`, called straight from the browser); both work, dispatched by the `server` state passed into `submitAutoAppPush`
